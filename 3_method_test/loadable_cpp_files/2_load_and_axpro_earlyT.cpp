...
pair<int, vector<vector<pair<int, double>>>> readFromFile(string filename) {
...
int main(int argc, char *argv[])
{
    ...
    // Read the adjacency list from a file
    pair<int, vector<vector<pair<int, double>>>> data = readFromFile(filename);
    int n = data.first;
    vector<vector<pair<int, double>>> adj = data.second;

    int src = 0; // the source vertex
    vector<double> dist(adj.size(), numeric_limits<double>::max()); // initialize all distances to infinity
    
    for (int j = 0; j < n; j++){
        int src = j;
        dijkstra_early_termination(src, adj, dist);
